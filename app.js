...
if (modelSection) modelSection.style.display = (mode === 'aiimage' || mode === 'motion') ? 'none' : 'block';
  ...
  const miniBtnDiv = document.querySelector('#uploadSection div[style*="flex-wrap"]');
...
function updateCreditDisplay() {
  const baseCost = CREDIT_COSTS[selectedModel]?.[selectedDuration] || 4;
...
function reportContent() {
  const subject = encodeURIComponent('FireMax - Content Report');
  const body = encodeURIComponent('I would like to report the following content generated by FireMax:\n\nDescription of issue:\n\nUser email: ' + (currentUser?.email || ''));
    ...
    grid.innerHTML = data.generations.map(item => `
      <div style="background:var(--surface); border-radius:12px; overflow:hidden; border:1px solid var(--border); width:100%; break-inside:avoid; margin-bottom:10px;">
        ...
        <div style="padding:8px; display:flex; flex-direction:column; gap:6px;">
          ...
          <button onclick="reportContent()" style="width:100%; margin-top:6px; background:transparent; border:1px solid #ff7777; color:#ff7777; border-radius:8px; padding:6px; font-size:12px; cursor:pointer;">⚠️ Report</button>
        </div>
      </div>
    `).join('');
